...
k_valve = 2                               #2.85 is unstable, 2.8 is stable. depends of course on parameters



#Greitzer coefficients calculated for a certain compressor (se aumenti B ottiene il ciclo limite)
# N = 5e3                                    #[rpm] --> large N enlarges the cirle
...
# Lt = 0.1                                    #throttle length [m]
# B_real = (U_ref/(2*a))*np.sqrt(Vp/(Ac*Lc))  #B parameter of the described compressor
# G_real = 10000*Lt*Ac/(Lc*At)                      #G parameter of the described compressor
# k_valve = 1                              #2.85 is unstable, 2.8 is stable. depends of course on parameters
...
fig.suptitle('Transient after throttle closure')
