...
n = 3
Dict = {}
x = np.linspace(0,np.pi/4,1000) #x plays the role of the beta2 variable
Vx = 1
Vy = 0 
Vy2 = 0.5
for ii in range(0,len(x)):
    Dict[x[ii]] = Brot_n(s, n, Vx, Vy1, Vy2, alfa1, beta1, x[ii], lambda_r, dLr_dTanb)
    
fig, axes = plt.subplots(3,3, figsize=format_fig)
fig.suptitle('Brot, s='+str(s.real)+'+'+str(s.imag)+'j')
axes[0,0].set_ylabel(r'$B_{rot_{11}}$')
axes[0,1].set_ylabel(r'$B_{rot_{12}}$')
axes[0,2].set_ylabel(r'$B_{rot_{13}}$')
axes[1,0].set_ylabel(r'$B_{rot_{21}}$')
axes[1,1].set_ylabel(r'$B_{rot_{22}}$')
axes[1,2].set_ylabel(r'$B_{rot_{23}}$')
axes[2,0].set_ylabel(r'$B_{rot_{31}}$')
axes[2,1].set_ylabel(r'$B_{rot_{32}}$')
axes[2,2].set_ylabel(r'$B_{rot_{33}}$')
...
fig.legend()
fig.savefig(path+'/Brot.png')

#%%B stator
s = 1+1j
n = 3
Dict = {}
x = np.linspace(0,np.pi/4,1000) #x plays the role of the alfa2 variable
Vx = 1
Vy = 0.5 
Vy2 = 1
for ii in range(0,len(x)):
    Dict[x[ii]] = Bsta_n(s, n, Vx, Vy1, Vy2, alfa1, x[ii], lambda_s, dLs_dTana)
    
fig, axes = plt.subplots(3,3, figsize=format_fig)
fig.suptitle('Bsta, s='+str(s.real)+'+'+str(s.imag)+'j')
axes[0,0].set_ylabel(r'$B_{sta_{11}}$')
axes[0,1].set_ylabel(r'$B_{sta_{12}}$')
axes[0,2].set_ylabel(r'$B_{sta_{13}}$')
axes[1,0].set_ylabel(r'$B_{sta_{21}}$')
axes[1,1].set_ylabel(r'$B_{sta_{22}}$')
axes[1,2].set_ylabel(r'$B_{sta_{23}}$')
axes[2,0].set_ylabel(r'$B_{sta_{31}}$')
axes[2,1].set_ylabel(r'$B_{sta_{32}}$')
axes[2,2].set_ylabel(r'$B_{sta_{33}}$')
T11 = np.zeros(len(x), dtype = complex)
T12 = np.zeros(len(x), dtype = complex)
T13 = np.zeros(len(x), dtype = complex)
T21 = np.zeros(len(x), dtype = complex)
